...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        ...
        case "all":
        case "-a": {
            const listAdmin = admins || config.admins || [];
            var msg = [];
        ...
        case "add": {
            ...
            if (mention.length != 0 && isNaN(content[0])) {
                ...
                admins.push(content[0]);
                config.admins.push(content[0]);
        ...
        case "delete": {
            ...
            if (mentions.length != 0 && isNaN(content[0])) {
                ...
                for (const id of mention) {
                    const index = config.admins.findIndex(item => item == id);
                ...
                const index = config.admins.findIndex(item => item.toString() == content[0]);
                admins.splice(index, 1);
                config.admins.splice(index, 1);
                const name = await Users.getNameUser(content[0]);
                writeFileSync(configPath, JSON.stringify(config, null, 2), 'utf8');
